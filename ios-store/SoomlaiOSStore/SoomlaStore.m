...
- (void)loadBillingService {
    if ([SKPaymentQueue canMakePayments]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(purchaseVerified:) name:EVENT_ID_MARKET_PURCHASE_VERIF object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(unexpectedVerificationError:) name:EVENT_ID_UNEXPECTED_STORE_ERROR object:nil];
...
- (void)purchaseVerified:(NSNotification*)notification {
    ...
    PurchasableVirtualItem* purchasable = [userInfo objectForKey:DICT_ELEMENT_ID_PURCHASABLE];
    BOOL verified = [(NSNumber*)[userInfo objectForKey:DICT_ELEMENT_ID_VERIFIED] boolValue];
    SKPaymentTransaction* transaction = [userInfo objectForKey:DICT_ELEMENT_ID_TRANSACTION];
    BOOL isRestored = [(NSNumber *)userInfo[DICT_ELEMENT_ID_IS_RESTORED] boolValue];
