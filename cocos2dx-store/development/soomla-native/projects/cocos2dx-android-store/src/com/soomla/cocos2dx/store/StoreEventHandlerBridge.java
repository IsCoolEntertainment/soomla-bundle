...
public class StoreEventHandlerBridge {
    ...
    public void pushOnPurchaseVerified(String itemType, String jsonPurchaseInfo, String signature) {
        try {
            BusProvider.getInstance().post
                (new PurchaseVerifiedEvent
                 (new IabPurchase(itemType, jsonPurchaseInfo, signature)));
        ...
        mGLThread.queueEvent(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    parameters.put("isRestored", itemPurchasedEvent.isRestored());
                    NdkGlue.getInstance().sendMessageWithParameters(parameters);
                } catch (JSONException e) {
                    throw new IllegalStateException(e);
                }
    ...
    @Subscribe
    public void onPurchaseVerificationStarted(final PurchaseVerificationStartedEvent event) {
        ...
        mGLThread.queueEvent(new Runnable() {
            ...
            public void run() {
                try {
                    JSONObject parameters = new JSONObject();
                    parameters.put("method", "CCStoreEventHandler::onPurchaseVerificationStarted");
                    parameters.put("itemType", event.Purchase.getItemType());
                    parameters.put("signature", event.Purchase.getSignature());
                    parameters.put("originalJson", event.Purchase.getOriginalJson());
        ...
        mGLThread.queueEvent(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    parameters.put("isRestored", marketPurchaseEvent.IsRestored);
