...
#define TAG "SOOMLA VirtualItemStorage"
    ...
    int CCVirtualItemStorage::getBalance(CCVirtualItem *item, CCError **error) {
        ...
        int balance = 0;
        if (val != NULL) {
            balance = cocos2d::__String::create(val)->intValue();
        }
        
        CCSoomlaUtils::logDebug(TAG, cocos2d::__String::createWithFormat("the balance for %s is %d", itemId, balance)->getCString());
    ...
    int CCVirtualItemStorage::setBalance(CCVirtualItem *item, int balance, bool notify, CCError **error) {
        ...
        const char *balanceStr = cocos2d::__String::createWithFormat("%d", balance)->getCString();
        const char *key = keyBalance(itemId);
    ...
    int CCVirtualItemStorage::add(CCVirtualItem *item, int amount, bool notify, CCError **error) {
        ...
        const char *balanceStr = cocos2d::__String::createWithFormat("%d", balance + amount)->getCString();
        const char *key = keyBalance(itemId);
