...
//            s_SharedVirtualGoodsStorage->retain();
    ...
    CCUpgradeVG *CCVirtualGoodsStorage::getCurrentUpgrade(CCVirtualGood *good, CCError **error) {
        ...
        if (upItemId == NULL) {
            CCSoomlaUtils::logDebug(TAG,
                                    cocos2d::__String::createWithFormat("You tried to fetch the current upgrade of %s but there's not upgrade to it.", itemId)->getCString());
            return NULL;
        }
    ...
    void CCVirtualGoodsStorage::postBalanceChangeEvent(CCVirtualItem *item, int balance, int amountAdded) {
        ...
        if (good == NULL) {
            CCSoomlaUtils::logError(TAG, cocos2d::__String::createWithFormat("Trying to post good balance changed with a non VirtualGood item %s", item->getId()->getCString())->getCString());
            return;
        }
    ...
    const char *CCVirtualGoodsStorage::keyGoodBalance(const char *itemId) {
        return cocos2d::__String::createWithFormat("good.%s.balance", itemId)->getCString();
    }
    
    const char *CCVirtualGoodsStorage::keyGoodEquipped(const char *itemId) {
        return cocos2d::__String::createWithFormat("good.%s.equipped", itemId)->getCString();
    }
    
    const char *CCVirtualGoodsStorage::keyGoodUpgrade(const char *itemId) {
        return cocos2d::__String::createWithFormat("good.%s.currentUpgrade", itemId)->getCString();
    }
