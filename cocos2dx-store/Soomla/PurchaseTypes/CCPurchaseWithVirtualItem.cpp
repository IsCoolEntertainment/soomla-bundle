...
namespace soomla {
    ...
    void CCPurchaseWithVirtualItem::buy(const char* payload, CCError **error) {
        ...
        if (associatedItem == NULL) {
            CCSoomlaUtils::logError(TAG, cocos2d::__String::createWithFormat("Trying to buy an non-existing associated item: %s", associatedItemId)->getCString());
            return;
        }
        
        const char *targetItemId = getItemId()->getCString();
        int amount = getAmount()->getValue();
        CCSoomlaUtils::logDebug(TAG, cocos2d::__String::createWithFormat("Trying to buy a %s with %d pieces of %s",
        ...
        if (item == NULL) {
            CCSoomlaUtils::logError(TAG, cocos2d::__String::createWithFormat("Target virtual item %s doesn't exist !", targetItemId)->getCString());
            return;
        }
        ...
        if (balance < amount){
            cocos2d::__String *errorStr = cocos2d::__String::createWithFormat("You tried to buy with itemId: %s but you don't have enough funds to buy it.",
        ...
        CCStoreEventDispatcher::getInstance()->onItemPurchased(associatedItem, cocos2d::__String::create(payload), true);
    }
    ...
    bool CCPurchaseWithVirtualItem::canAfford(CCError **error) {
        ...
        const char *targetItemId = getItemId()->getCString();
        int amount = getAmount()->getValue();
        CCSoomlaUtils::logDebug(TAG, cocos2d::__String::createWithFormat("Checking affordability of %s with %d pieces of %s",
                                                                associatedItem->getName()->getCString(),
                                                                amount,
                                                                targetItemId)->getCString());
