...
namespace soomla {
    ...
    void CCSoomlaStore::initialize(CCStoreAssets *storeAssets, cocos2d::__Dictionary *storeParams) {
        ...
        if (initialized) {
            CCStoreEventDispatcher::getInstance()->onUnexpectedStoreError(cocos2d::__Integer::create(0), true);
    ...
    void CCSoomlaStore::buyMarketItem(const char *productId, const char *payload, CCError **error) {
        ...
        CCStoreEventDispatcher::getInstance()->onMarketPurchase(item, cocos2d::__Bool::create(false), cocos2d::__String::create(payload), cocos2d::__Dictionary::create());

        item->give(1);

        //complete purchasing routine
        CCStoreEventDispatcher::getInstance()->onItemPurchased(item, cocos2d::__String::create(payload));
    }
#undef TAG
}
