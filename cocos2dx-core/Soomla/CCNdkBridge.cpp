...
namespace soomla {
    extern "C"
    {
        ...
        {
            std::string jsonString = cocos2d::JniHelper::jstring2string(json);
...
#if (LOG_JSON == 1)
            CCSoomlaUtils::logDebug("CCNdkBridge", cocos2d::__String::createWithFormat(
...
#if (LOG_JSON == 1)
            CCSoomlaUtils::logDebug(TAG, cocos2d::__String::createWithFormat(
                    "from native JSON: %s", json_dumps(retJsonParams, JSON_COMPACT | JSON_ENSURE_ASCII))->getCString());
#endif

            json_decref(toBeSentJson);
            cocos2d::Ref *retParams = CCJsonHelper::getCCObjectFromJson(retJsonParams);
