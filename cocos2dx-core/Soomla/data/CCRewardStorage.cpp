...
namespace soomla {
    ...
    int CCRewardStorage::getTimesGiven(CCReward *reward) {
        ...
        return cocos2d::__String::create(val)->intValue();
    }
    ...
    void CCRewardStorage::setLastSeqIdxGiven(CCSequenceReward *sequenceReward, unsigned int idx) {
        const char *key = keyRewardIdxSeqGiven(sequenceReward->getId()->getCString());
        CCKeyValueStorage::getInstance()->setValue(key, cocos2d::__String::createWithFormat("%d", idx)->getCString());
    }
    ...
    void CCRewardStorage::setTimesGiven(CCReward *reward, bool up, bool notify) {
        ...
        const char *key = keyRewardTimesGiven(reward->getId()->getCString());
        CCKeyValueStorage::getInstance()->setValue(key, cocos2d::__String::createWithFormat("%d", total)->getCString());
        ...
        if (up) {
            ...
            time_t currentTime;
            time(&currentTime);
            cocos2d::__String *val = cocos2d::__String::createWithFormat("%lld", (long long)(((double)currentTime) * 1000));
    ...
    const char* CCRewardStorage::keyRewards(const char* rewardId, const char* postfix) {
        return cocos2d::__String::createWithFormat("%srewards.%s.%s", DB_KEY_PRFIX, rewardId, postfix)->getCString();
    }
