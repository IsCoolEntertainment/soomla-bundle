...
import com.soomla.store.events.PurchaseVerificationStartedEvent;
import com.soomla.store.events.PurchaseVerifiedEvent;
...
public class SoomlaStore {
    ...
    private void handleSuccessfulPurchase(IabPurchase purchase, boolean isRestoring )    {
        BusProvider.getInstance().post
            (new PurchaseVerificationStartedEvent(purchase));
        if( isRestoring ) {
            mRestoringList.add( purchase.getOrderId() );
        }
                
    }

    @Subscribe
    public void purchaseVerified(final PurchaseVerifiedEvent event) {
        final IabPurchase purchase = event.Purchase;
        String sku = purchase.getSku();
        ...
        final boolean isRestoring = mRestoringList.remove( orderId );
    ...
    private void finalizeTransaction(IabPurchase purchase, PurchasableVirtualItem pvi, boolean isRestoring) {
        ...
        consumeInBillingService(purchase, pvi);
    }

    private void consumeInBillingService( final IabPurchase purchase, final PurchasableVirtualItem pvi ) {

        final String itemId = pvi.getItemId();
        final String developerPayload = purchase.getDeveloperPayload();
    
        if (StoreInfo.isItemNonConsumable(pvi)) {
            BusProvider.getInstance().post(new ItemPurchasedEvent(itemId, developerPayload));
            return;
        }
        ...
        mInAppBillingService.initializeBillingService(
                new IabCallbacks.IabInitListener() {
                    ...
                    public void success(boolean alreadyInBg) {
                        ...
                        SoomlaUtils.LogDebug(TAG,
                                "Setup successful, consuming purchases");
    ...
    private List<String> mRestoringList = new ArrayList<String>();
